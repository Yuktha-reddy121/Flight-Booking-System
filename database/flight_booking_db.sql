...
COMMIT;
/*
SELECT * FROM airport_list order by airport asc;
SELECT * FROM airport_list;
SELECT * FROM airport_list order by id asc;
SELECT f.*,a.airlines,a.logo_path FROM flight_list f inner join airlines_list a on f.airline_id = a.id $where order by rand();
SELECT f.*,a.airlines,a.logo_path FROM flight_list f inner join airlines_list a on f.airline_id = a.id  order by id desc;
SELECT b.*,f.*,a.airlines,a.logo_path,b.id as bid FROM  booked_flight b inner join flight_list f on f.id = b.flight_id inner join airlines_list a on f.airline_id = a.id  order by b.id desc;
SELECT * FROM booked_flight where flight_id = ".$row['id']";
SELECT * FROM flight_list where id=".$_GET['id']";
SELECT * FROM system_settings limit 1;
SELECT * FROM users where id =".$_GET['id']";
SELECT * FROM users order by name asc;
SELECT * FROM users where username = '".$username."' and password = '".$password." ' ;
SELECT * FROM users where username = '".$email."' and password = '".md5($password)."';
...
INSERT INTO airlines_list set ;
UPDATE airlines_list set ".$data." where id=".$id";
DELETE FROM airlines_list where id = ".$id";

INSERT INTO airport_list set ".$data";
UPDATE airport_list set ".$data." where id=".$id";
DELETE FROM airport_list where id = ".$id";

INSERT INTO flight_list set ".$data";
UPDATE flight_list set ".$data." where id=".$id";
DELETE FROM flight_list where id = ".$id";

INSERT INTO booked_flight set ".$data";
UPDATE booked_flight set ".$data." where id =".$id";
...
DELIMITER//
CREATE PROCEDURE get_all_airliness
(IN id int(30))
BEGIN
  SELECT * FROM 'airlines_list' WHERE a.id=id;
end//
DELIMITER;

call get_all_airliness(2)


DELIMITER//
CREATE PROCEDURE get_all_airliness_booking
(IN id  int(30))
BEGIN
SELECT * FROM 'airlines_list','booked_flight','flight_list' where f.id=b.flight_id and b.id=a.id and a.id=id;
END//
DELIMITER;

CALL get_all_airliness_booking(2) 
...
BEGIN
  ...
  SET
  lb_booked_id=NEW.booked_id,
  lb_date=NOW(),
  lb_action="flight successfully booked";
end$$
DELIMITER;
...
CREATE TRIGGER delete_airlines
 ...
 FOR EACH ROW
BEGIN
   DELETE FROM flight_list where f.airline_id=OLD.airline_id;
end$$
delimiter;
...
BEGIN
    ...
    lb_booked_id=NEW.booked_id,
    lb_date=NOW(),
    lb_action="booked flight successfully updated"
;
END$$
DELIMITER;	
*/
